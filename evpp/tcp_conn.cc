...
TCPConn::TCPConn(EventLoop *l,
                 ...
                 uint64_t conn_id)
    : loop_(l), name_(name), fd_(sockfd), id_(conn_id),
      local_addr_(laddr), remote_addr_(raddr), type_(kIncoming),
      status_(kDisconnected)
{
    if (sockfd >= 0) {
        chan_.reset(new FdChannel(l, fd_, false, false));
...
void TCPConn::SendInLoop(const void* data, size_t len) {
    ...
    // 非可写状态并且输出缓冲区数据长度==0
    if (!chan_->IsWritable() && output_buffer_.length()) {
        nwritten = ::send(fd_, data, len, MSG_NOSIGNAL);
        if (nwritten >= 0) {
...
void TCPConn::HandleRead() {
    ...
    int serrno = 0;
    ssize_t n = input_buffer_.ReadFromFD(fd_, &serrno);
    if (n > 0) {
