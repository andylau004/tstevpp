...
FdChannel::FdChannel(EventLoop* l, evpp_socket_t fd, bool r, bool w)
    : loop_(l), attached_(false), event_(nullptr), fd_(fd)
{
    DLOG_TRACE << "fd=" << fd_;
    assert(fd_ > 0);
...
std::string FdChannel::EventsToString() {
    std::string s;
...
void FdChannel::HandleEvent(evpp_socket_t sockfd, short which, void *v) {
    FdChannel* pthis = (FdChannel*)v;
    pthis->HandleEvent(sockfd, which);
}
