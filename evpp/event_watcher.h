...
public:
    ...
    TimerEventWatcher(struct event_base* inbase, const Handler& handler, Duration timeout);
    TimerEventWatcher(struct event_base* inbase, Handler&& handler, Duration timeout);
    bool AsyncWait();
private:
    virtual bool DoInit();
    static void HandlerFn(evpp_socket_t fd, short which, void* v);
...
public:
    ...
    SignalEventWatcher(signal_number_t signo, EventLoop* loop, Handler&& h);
