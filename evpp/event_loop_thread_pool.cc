...
bool EventLoopThreadPool::Start(bool wait_thread_started) {
    ...
    std::shared_ptr< std::atomic<int> > start_count( new std::atomic<int>(0) );
    std::shared_ptr< std::atomic<int> > exit_count( new std::atomic<int>(0) );

    for (uint32_t i = 0; i < thread_num_; ++ i) {
        auto prefn = [this, start_count]() {
            DLOG_TRACE << "a working thread started tid=" << std::this_thread::get_id();
            this->OnThreadStarted(start_count->fetch_add(1) + 1);
            return EventLoopThread::kOK;
        };
        auto postfn = [this, exit_count]() {
            DLOG_TRACE << "a working thread exiting tid=" << std::this_thread::get_id();
            this->OnThreadExited(exit_count->fetch_add(1) + 1);
            return EventLoopThread::kOK;
        };
