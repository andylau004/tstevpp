...
void TCPServer::StopInLoop(DoneCallback on_stopped_cb) {
    ...
    if (connections_.empty()) {
        ...
        DLOG_TRACE << "close all connections";
...
#endif
    ++next_conn_id_;
    TCPConnPtr newCnn(new TCPConn(io_loop, n, sockfd, listen_addr_, remote_addr, next_conn_id_ ));
    assert(newCnn->type() == TCPConn::kIncoming);
//    LOG_INFO << "newCnn use_count=" << newCnn.use_count();

    newCnn->SetConnectionCallback(conn_fn_);
    newCnn->SetMessageCallback(msg_fn_);
    newCnn->SetCloseCallback(std::bind(&TCPServer::RemoveConnection, this, std::placeholders::_1));

    io_loop->RunInLoop(std::bind(&TCPConn::OnAttachedToLoop, newCnn));
//    LOG_INFO << "newCnn use_count=" << newCnn.use_count();

    connections_[newCnn->id()] = newCnn;
    LOG_INFO << "newCnn use_count=" << newCnn.use_count();
}
